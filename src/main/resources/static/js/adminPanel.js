 ...
 * отладкой
 * *********************************************************************************************************************
 * Для увеличения модульности можно создать класс - конструктор запроса с использование паттерна builder и гибко
 * настройть такие параметры запроса, как строка запроса, метод, headers, body
 * ********************************************************************************************************************
 * Так же можно было создать единый обработчик событий нажатия кнопок, получать созданные по определенному типу индексы
 * необоходимых обьектов(например : Префикс+разделитель+типкнопки+id), парсить строку и использовать оператор Swich-case
 * в блоках которого вызывать функции с переданными необохдимыми параметрами для дальшего создания пепременных, инстанцирования обьктов,
 * иной предобработки данных, после чего уже внутри вызванной функции создавать экземпляр класса запроса, передать все
 * необходимые данные и вызвать метод fetch, после чего вызвать ещё одну отдельно созданную функцию в которой будут храниться
 * методы для пост обработки запроса/
